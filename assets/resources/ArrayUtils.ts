...
export class ArrayUtils {
    ...
    public static find(collection: any, predicate: Function) {
        ...
        if (!Array.isArray(collection)) {
            collection = ArrayUtils._toArray(collection);
        }
    ...
    public static forEach(collection: any, iteratee: any) {
        if (!Array.isArray(collection)) {
            var array = ArrayUtils._toArrayKey(collection);
    ...
    public static cloneDeep(sObj: any) {
        ...
        for (let i in sObj) {
            if (sObj.hasOwnProperty(i)) {
                s[i] = ArrayUtils.cloneDeep(sObj[i]);
            }
        }
    ...
    public static deepCopy(o) {
        ...
        for (key in o) {
            v = o[key];
            out[key] = (typeof v === "object") ? ArrayUtils.deepCopy(v) : v;
        }
        return out;
    }
    ...
    public static isEqual(x: any, y: any): boolean {
        ...
        for (var p in x) {
            ...
            if (a && b) {
                return ArrayUtils.isEqual(x[p], y[p]);
            } else if (x[p] !== y[p]) {
                return false;
            }
        }
    ...
    public static remove(array: any[], predicate: Function) {
        ...
        ArrayUtils._basePullAt(array, indexes);
        return result;
    }
    ...
    public static flattenDeep(arr: any[]): any {
        return arr.reduce(function (prev: any[], cur: any[]) {
            return prev.concat(Array.isArray(cur) ? ArrayUtils.flattenDeep(cur) : cur);
        }, []);
    }
    ...
    public static isNaN(value: any) {
        ...
        return ArrayUtils.isNumber(value) && value !== +value;
    }
    ...
    public static isLength(value: any) {
        return typeof value === 'number' &&
            value > -1 && value % 1 === 0 && value <= ArrayUtils.MAX_SAFE_INTEGER;
    }

    public static _isArrayLike(value: []) {
        return value !== null && ArrayUtils.isLength(value.length) /*&& !isFunction(value)*/;
    }
